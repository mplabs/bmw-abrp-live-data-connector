...
const fakeClient = {
    ...
    subscribe: (topic: string, options: Record<string, unknown>, callback?: (err?: Error | null) => void) => {
...
describe('connectBmwMqtt', () => {
    ...
    it('can use access token for MQTT password when configured', async () => {
        const { connectBmwMqtt } = await import('../src/bmw/mqtt')
    ...
    it('uses explicit mqtt username/password overrides when provided', async () => {
        const { connectBmwMqtt } = await import('../src/bmw/mqtt')
